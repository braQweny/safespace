...
Konto `free` może posiadać łącznie najwyżej `FREE_PLAN_SESSION_LIMIT` (3) sesji — liczy się każdy wiersz `therapy_sessions` właściciela, niezależnie od statusu, także tombstone po usunięciu. Konto `premium` nie ma limitu liczby rozmów, każda ma budżet 3600 sekund. Po wygaśnięciu opłaconego okresu obowiązuje plan darmowy bez oczekiwania na webhook; rozpoczęta rozmowa zachowuje przypisany budżet. Rola `safespace_billing` nie ma dostępu do tabel rozmów, wiadomości ani podsumowań.
