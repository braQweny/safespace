...
const SESSION_START_CARD_COPY = defineCopy(
  {
    ...
    startVoiceTrial: "Start a voice conversation",
    preparingVoice: "Preparing the voice conversation…",
    voiceIntro: (name: string) =>
      `You talk out loud and ${name} answers in a voice. The transcript stays in your history, just like after a written conversation.`,
    voiceTrialBudget: (minutes: string) => `One free voice conversation, up to ${minutes}`,
    // Próbę i czas zużywa pierwsze połączenie audio, nie sam start.
    voiceTrialNote: "The time counts from when you turn on the microphone.",
  ...
  },
  {
    ...
    startVoiceTrial: "Zacznij rozmowę głosową",
    preparingVoice: "Przygotowujemy rozmowę głosową…",
    voiceIntro: (name) =>
      `Mówisz na głos, ${name} odpowiada głosem. Zapis zostaje w historii, jak po rozmowie pisanej.`,
    voiceTrialBudget: (minutes) => `Jedna bezpłatna rozmowa głosowa, do ${minutes}`,
    voiceTrialNote: "Czas liczy się od włączenia mikrofonu.",
