...
describe("auth forms (server render)", () => {
  it("render an empty e-mail field even when a server error asks for a restore", () => {
    for (const markup of [
      renderToStaticMarkup(<SignInForm serverError="Podaj poprawny adres e-mail." />),
      renderToStaticMarkup(<SignUpForm serverError="Podaj poprawny adres e-mail." />),
    ]) {
  ...
  it("keep the native POST targets so the forms work without JavaScript", () => {
    expect(renderToStaticMarkup(<SignInForm />)).toContain('action="/api/auth/signin"');
    expect(renderToStaticMarkup(<SignUpForm />)).toContain('action="/api/auth/signup"');
  });
});
