...
- **`session-data/`** — owner-bound repository over private tables (`therapy_sessions`, `session_messages`, `session_summaries`, `session_trial_claims`). Entry point `getSessionDataContext()`. Free-trial start _must_ go through `claimFreeTrialSession()` (DB unique constraint, not a UI check); the free-plan session cap is read through `readSessionQuota()` (`quota.ts`) and enforced by a DB trigger (see "Account plans" above). Deletes go through `deleteOwnedSession()` (hard-deletes content, leaves a content-free tombstone). Hand-maintained domain types in `types.ts` are the privacy contract — see its README before adding Supabase typegen.
