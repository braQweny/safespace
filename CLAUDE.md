...
6. Stamps baseline security headers on every response (X-Frame-Options, HSTS, Referrer-Policy, Permissions-Policy…). **The CSP itself comes from Astro** (`security.csp` in `astro.config.mjs`), built at compile time from the real island/style hashes — the middleware does not set it. Two directives there are load-bearing and easy to break:
