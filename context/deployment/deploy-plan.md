...
- `.github/workflows/ci.yml`:
  ...
  - Rozmowa głosowa: pierwszy deploy z własnym `main` tworzy klasę Durable Object `VoiceSessionObserver` (migracja `v1`, `new_sqlite_classes`) i limiter `VOICE_RATE_LIMITER`; obiekt czyta `OPENAI_API_KEY` i `OPENROUTER_SAFETY_MODEL` z sekretów i `vars` Workera. `VOICE_SESSION_MODE=off` i `VOICE_MONTHLY_MINUTES=120` są w `vars`. Włączenie to osobny commit zmieniający tylko flagę, po bramkach manualnych z listy powyżej; wyłączenie w trakcie rozmów rozłącza trwające sesje live przy najbliższym heartbeacie (`voice_disabled`), zapisane transkrypty zostają.
