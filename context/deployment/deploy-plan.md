...
- Rozmowa głosowa (GPT-Live-1): `VOICE_SESSION_MODE` jest `on` w `wrangler.jsonc` od 2026-09-19 decyzją właściciela, przy otwartych bramkach manualnych. Do nadrobienia na włączonej funkcji: potwierdzić w panelu OpenAI zerową retencję (ZDR) dla sesji live i region konta (spike S6), wykonać bieg 60 minut obserwatora na prawdziwym runtime Cloudflare po `npx wrangler login` (S12/S13), odsłuchać polskie nagrania (S3), sprawdzić telefony (S9) i przejść listę manualną z `context/changes/voice-live-conversation/verification.md`. Flaga działa tylko przy `AI_PROVIDER=openai`; powrót do OpenRouter gasi start głosowy bez zmiany flagi.
...
- `.github/workflows/ci.yml`:
  ...
  - Migracja `20260912200000` dodaje tryb rozmowy (`mode`), aktywację połączenia głosowego, bucket 600 s, osobną próbę głosową konta free (`P0016`), `utterance_id` i RPC `append_voice_session_utterances`. Jest addytywna i kompatybilna z dotychczasowym Workerem (domyślne `mode = 'text'`, limit tekstowy bez zmian); musi wejść przed Workerem czytającym nowe kolumny. Nie dodaje sekretów.
  - Migracja `20260923120000` przenosi zużycie próby głosowej konta free i start zegara rozmowy głosowej na pierwsze połączenie audio: bramka `P0016` liczy tylko wiersze połączone albo trwające, a trigger metadanych przy pierwszym `voice_connected_at` przesuwa `started_at`/`expires_at` (długość bez zmian). Zgodna z dotychczasowym Workerem: stary `connect` uzbraja obserwatora terminem sprzed przesunięcia (wcześniejszym), więc rozmowa kończy się najpóźniej tam, gdzie dotąd; stary panel liczy próbę surowiej niż trigger. Musi wejść przed Workerem, który czyta przesunięte okno. Nie dodaje sekretów.
  - Rozmowa głosowa: pierwszy deploy z własnym `main` tworzy klasę Durable Object `VoiceSessionObserver` (migracja `v1`, `new_sqlite_classes`) i limiter `VOICE_RATE_LIMITER`; obiekt czyta `OPENAI_API_KEY` i `OPENROUTER_SAFETY_MODEL` z sekretów i `vars` Workera. `VOICE_SESSION_MODE=on` (od 2026-09-19) i `VOICE_MONTHLY_MINUTES=60` są w `vars` (60, nie 120: ok. 0,20 zł za minutę GPT-Live-1 z zapleczem, wycena 2026-09-13). Wyłączenie to commit zmieniający flagę na `off` i ponowny deploy (flaga jest wkompilowana w build); wyłączenie w trakcie rozmów rozłącza trwające sesje live przy najbliższym heartbeacie (`voice_disabled`), zapisane transkrypty zostają.
