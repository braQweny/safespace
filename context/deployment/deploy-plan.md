...
- Rozmowa głosowa (GPT-Live-1) jest wdrożona „na ciemno”: `VOICE_SESSION_MODE` zostaje `off` w `wrangler.jsonc`. Przed osobnym commitem przełączającym flagę na `on`: potwierdzić w panelu OpenAI zerową retencję (ZDR) dla sesji live i region konta (spike S6), wykonać bieg 60 minut obserwatora na prawdziwym runtime Cloudflare po `npx wrangler login` (S12/S13), odsłuchać polskie nagrania (S3), sprawdzić telefony (S9) i przejść listę manualną z `context/changes/voice-live-conversation/verification.md`. Flaga działa tylko przy `AI_PROVIDER=openai`; powrót do OpenRouter gasi start głosowy bez zmiany flagi.
...
- `.github/workflows/ci.yml`:
  ...
  - Rozmowa głosowa: pierwszy deploy z własnym `main` tworzy klasę Durable Object `VoiceSessionObserver` (migracja `v1`, `new_sqlite_classes`) i limiter `VOICE_RATE_LIMITER`; obiekt czyta `OPENAI_API_KEY` i `OPENROUTER_SAFETY_MODEL` z sekretów i `vars` Workera. `VOICE_SESSION_MODE=off` i `VOICE_MONTHLY_MINUTES=60` są w `vars` (60, nie 120: ok. 0,20 zł za minutę GPT-Live-1 z zapleczem, wycena 2026-09-13). Włączenie to osobny commit zmieniający tylko flagę, po bramkach manualnych z listy powyżej; wyłączenie w trakcie rozmów rozłącza trwające sesje live przy najbliższym heartbeacie (`voice_disabled`), zapisane transkrypty zostają.
